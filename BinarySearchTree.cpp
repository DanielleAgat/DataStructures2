...
namespace DataQ2 {
    ...
    BinSearchNode::BinSearchNode(const BinSearchNode *toCopy) {
        *this = toCopy;
    }
    ...
    BinarySearchTree &BinarySearchTree::operator=(const BinarySearchTree &toCopy) {
        if (this != &toCopy) {
            if (root != nullptr)
                makeEmpty();
            if (toCopy.root == nullptr)
                ...
                BinSearchNode *copyNode = new BinSearchNode();
                copyNode->data = toCopy.root->data;
                copyNode->left = copyBinarySearchTree(toCopy.root->left);
                copyNode->right = copyBinarySearchTree(toCopy.root->right);
                root = copyNode;
            }
        }
        return *this;
    ...
    BinSearchNode *BinarySearchTree::copyBinarySearchTree(const BinSearchNode *toCopy) const {
        if (toCopy == nullptr)
            return nullptr;
        BinSearchNode *copyNode = new BinSearchNode(nullptr, nullptr, toCopy->data);
        copyNode->left = copyBinarySearchTree(toCopy->left);
        copyNode->right = copyBinarySearchTree(toCopy->right);
        return copyNode;
    }
    ...
    BinSearchNode &BinSearchNode::operator=(const BinSearchNode &toCopy) {
        if (this != &toCopy) {
            BinSearchNode copyNode;
            data = toCopy.data;
            if (toCopy.left != nullptr)
                left = new BinSearchNode(*toCopy.left);
            else
                left = nullptr;
            if (toCopy.right != nullptr)
                right = new BinSearchNode(*toCopy.right);
            else
                right = nullptr;
        }
        return *this;
